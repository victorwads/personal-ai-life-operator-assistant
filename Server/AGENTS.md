...
* Keep files focused and small; avoid files approaching 500 lines.
* Prefer one primary type per file.
* Move shared enums, state structs, DTOs, protocols, and models to their appropriate feature folders.
* Keep services focused on orchestration; extract helpers or collaborators when responsibilities grow.
* Keep methods short and intention-revealing; split long methods before adding more behavior.
* Do not add new responsibilities to already oversized files.
* Put detailed module rules in the nearest `Architecture.md`, not in this file.
